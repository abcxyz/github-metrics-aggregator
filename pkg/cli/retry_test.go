...
func TestRetryJobCommand(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			expErr: `invalid configuration: GITHUB_PRIVATE_KEY_SECRET or GITHUB_PRIVATE_KEY_KMS_KEY_ID is required`,
		},
		{
			name: "invalid_config_bucket_url",
			env: map[string]string{
				"GITHUB_APP_ID":             "test-github-app-id",
				"GITHUB_PRIVATE_KEY_SECRET": "test-github-private-key",
			},
		...
		},
		{
			...
			expErr: `invalid configuration: only one of GITHUB_PRIVATE_KEY_SECRET, GITHUB_PRIVATE_KEY_KMS_KEY_ID is required`,
		},
		{
			name: "happy_path",
			env: map[string]string{

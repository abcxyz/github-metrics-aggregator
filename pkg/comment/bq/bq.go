...
func Query[T any](ctx context.Context, client *bigquery.Client, queryString string, project string, dataset string, parameters []bigquery.QueryParameter) ([]T, error) {
	query := client.Query(queryString)
	query.Parameters = parameters
	// Identifiers cannot be parameters, so this needs to be injected in client rather than sql
